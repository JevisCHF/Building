...
import Scrapy_ChinaBaogao_V1_01.proxy as Proxy
import logging
...
class ProxyMiddleWare(object):
    ...
    def process_request(self, request, spider):
        # 对 request 加上 proxy
        # proxy = get_proxy()
        request.meta['proxy'] = self.proxy['http']
        ...
        try:
            if str(response.status).startswith('4') or str(response.status).startswith('5'):
                ...
                while True:
                    ...
                    if self.proxy['http'] in self.invalid_proxy:
                        self.proxy = Proxy.get_proxy()
                        continue
                    else:
                        request.meta['proxy'] = self.proxy['http']
                        logging.debug('is the invalid ip, replace ip:' + self.proxy['http'])
                        # 对当前request 加上代理
                        break
                return request
        except:
            request.meta['proxy'] = self.proxy['http']
            logging.info('this is response ip:' + self.proxy['http'])
            # 对当前 request 加上代理
            return request
    ...
    def process_exception(self, request, exception, spider):
        ...
        while True:
            ...
            if self.proxy['http'] in self.invalid_proxy:
                ...
                request.meta['proxy'] = self.proxy['http']
                logging.debug('there is an error, replace ip:' + self.proxy['http'])
                # 对当前request 加上代理
                break
        return request
